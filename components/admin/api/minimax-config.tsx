...
export function MinimaxConfig({ configs }: { configs: any[] }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast({ description: 'MiniMax 配置添加成功' })
        setShowDialog(false)
        setFormData({ config_name: '', group_id: '', api_key: '' })
        router.refresh()
      } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {configs.map((config) => (
          <Card key={config.id}>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">Group ID: {config.group_id || '未配置'}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>添加 MiniMax 配置</DialogTitle>
