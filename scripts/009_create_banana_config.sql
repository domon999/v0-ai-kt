...
-- RLS 策略：任何人都可以读取活跃配置
CREATE POLICY "banana_config_select_active" ON public.banana_config
  FOR SELECT
  USING (is_active = true);

-- RLS 策略：只有管理员可以管理配置
CREATE POLICY "banana_config_admin_manage" ON public.banana_config
  FOR ALL
  USING (
    EXISTS (
...
INSERT INTO public.banana_config (config_name, api_key, base_url, model_key, is_active, priority, notes) VALUES
  ('primary', '', 'https://www.blueshirtmap.com/v1/chat/completions', 'gemini-3-pro-image-preview', false, 1, '主力接口 - 请在管理后台配置 API Key'),
  ('backup', '', 'https://gap.blueshirtmap.com/v1/chat/completions', 'gemini-3-pro-image-preview', false, 2, '备用接口 - 请在管理后台配置 API Key')
